...
:::caution[AWS Permissions Required] Your AWS user/role needs these permissions:
...
Understanding the Kubrick project structure will help with customization and
troubleshooting:

<FileTree>
  - kubrick/ - cli/ # Command-line interface tool - src/ - package.json -
  lambda/ # Serverless functions - src/ - api_fetch_tasks_handler/ -
  api_fetch_videos_handler/ - api_search_handler/ -
  api_video_upload_link_handler/ - db_bootstrap/ - s3_delete_handler/ -
  sqs_embedding_task_consumer/ - sqs_embedding_task_producer/ - layers/ -
  config_layer/ - embed_service_layer/ - response_utils_layer/ - s3_utils_layer/
  - vector_database_layer/ - build-package.sh # Lambda build script -
  playground/ # Next.js frontend - src/ - package.json - terraform/ #
  Infrastructure as Code - modules/ - api_gateway/ - cloudfront/ - dynamodb/ -
  iam/ - lambda/ - rds/ - s3/ - s3_notifications/ - secrets_manager/ - sqs/ -
  vpc_network/ - main.tf - variables.tf - terraform.tfvars.example -
  build-all-packages.sh # Build all Lambda packages - README.md
</FileTree>
...
2. **Build Lambda Packages**
   ...
   Build all serverless functions and layers:
   ...
   This script:
   - Creates Lambda deployment packages for all functions
   - Builds shared layers with dependencies
   - Uses `uv pip install` for Python dependency management
   - Ensures x86_64 Linux compatibility
...
4. **Configure Variables**
   ...
   :::tip[Variable Reference] All available variables are defined in
   `variables.tf`:
   - `aws_region` - AWS region for deployment (default: us-east-2)
   - `secret_name` - Secrets Manager secret name (default: kubrick_secret)
...
4. **Verify Import**
   ...
   ```bash
   terraform plan  # Should show no changes if properly configured
   ```
...
aws logs describe-log-groups --log-group-name-prefix "/aws/lambda/kubrick"
...
:::danger[Data Loss Warning] This will permanently delete all data including
uploaded videos and database contents. :::
...
```bash
# Remove CloudFront (takes 15+ minutes)
terraform destroy -target=module.cloudfront

# Remove RDS (with final snapshot)
terraform destroy -target=module.rds
```

### Cost Optimization

Monitor AWS costs and consider:

- RDS instance sizing
- CloudFront usage patterns
- Lambda memory allocation
- S3 storage classes for video archives
...
- **API Integration**: Explore the [API Reference](/api/reference)
- **Architecture Details**: Read the [Case Study](/case-study/intro)
- **CLI Tool**: Try the [Quick Start](/guides/quick-start) for simpler
  deployments
