...
The trade-off is a slight increase in code-complexity \- the consumer Lambda,
for example, has to explicitly and idempotently manage SQS visibility timeouts
and return pending messages to the queue.
...
(\<10ms vs \<1ms) were relatively small compared to the latency of a cache-miss
(\>1000ms). Search request parameters are used as the cache-key, to look-up and
store vector embeddings returned from the external embedding model.
